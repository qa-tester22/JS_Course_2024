...
Обзор: асинхронный
Следующий
Промисы — это основа асинхронного программирования в современном JavaScript. Обещание — это объект, возвращаемый асинхронной функцией, который представляет текущее состояние операции. В момент возврата обещания вызывающей стороне операция часто не завершается, но объект обещания предоставляет методы для обработки возможного успеха или неудачи операции.

Предпосылки:	Разумное понимание основ JavaScript, включая обработку событий.
Цель:	Понять, как использовать обещания в JavaScript.
...
При использовании API на основе обещаний асинхронная функция запускает операцию и возвращает объект Promise. Затем вы можете прикрепить обработчики к этому объекту обещания, и эти обработчики будут выполняться в случае успеха или неудачи операции.
...
вызов fetch()API и присвоение возвращаемого значения fetchPromiseпеременной
сразу после регистрации fetchPromiseпеременной. Это должно вывести что-то вроде: Promise { <state>: "pending" }, сообщающее нам, что у нас есть Promiseобъект, и его stateзначение равно "pending". Состояние "pending"означает, что операция выборки все еще продолжается.
...
Для поддержки обработки ошибок Promiseобъекты предоставляют catch()метод. Это очень похоже на then(): вы вызываете его и передаете функцию-обработчик. Однако, хотя переданный обработчик вызывается при успешном выполненииthen() асинхронной операции , переданный обработчик вызывается при сбое асинхронной операции .catch()
...
Статья «Давайте поговорим о том, как говорить об обещаниях» дает отличное объяснение деталей этой терминологии.
...
Иногда нужно, чтобы все обещания были выполнены, но они не зависят друг от друга. В таком случае гораздо эффективнее начать их все вместе, а затем получить уведомление, когда все они будут выполнены. Метод Promise.all()– вот что вам нужно. Он принимает массив обещаний и возвращает одно обещание.
...
Это всего лишь две дополнительные Promiseфункции для объединения нескольких промисов. Об остальном читайте в Promiseсправочной документации.

асинхронно и ждать
Ключевое asyncслово дает вам более простой способ работы с асинхронным кодом на основе обещаний. Добавление asyncв начало функции делает ее асинхронной:
...
fetchProducts();
Здесь мы вызываем await fetch(), и вместо того, чтобы получить Promise, наш вызывающий объект возвращает полностью завершенный Responseобъект, как если бы это fetch()была синхронная функция!

Мы даже можем использовать try...catchблок для обработки ошибок, точно так же, как если бы код был синхронным.
...
Также обратите внимание, что вы можете использовать только awaitвнутри asyncфункции, если только ваш код не находится в модуле JavaScript . Это означает, что вы не можете сделать это в обычном сценарии:
...
Вероятно, вы будете asyncчасто использовать функции там, где в противном случае вы могли бы использовать цепочки обещаний, и они делают работу с обещаниями намного более интуитивно понятной.

Имейте в виду, что, как и цепочка обещаний, awaitасинхронные операции выполняются последовательно. Это необходимо, если результат следующей операции зависит от результата последней, но если это не так, то что-то вроде Promise.all()будет более производительным.
...
Ключевые слова asyncи awaitупрощают построение операции из серии последовательных вызовов асинхронных функций, избегая необходимости создавать явные цепочки обещаний и позволяя писать код, который выглядит так же, как синхронный код.
