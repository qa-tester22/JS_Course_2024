...
Например, у нас 
есть объект user со своими свойствами и методами, и мы хотим 
создать объекты admin и guest как его слегка изменённые варианты. 
...
В JavaScript объекты имеют специальное скрытое свойство [[Prototype]] (так оно названо в спецификации), которое либо равно null, либо ссылается на другой объект. Этот объект называется «прототип»:
      ...
      прототип object
            ...
            |______________|      
                 /|\
                  |  [[Prototype]]
 object    _______|__________
          |__________________|

Прототип даёт нам немного «магии». 
Когда мы хотим прочитать свойство из object, а оно отсутствует, JavaScript автоматически берёт его из прототипа. В программировании такой механизм называется 
...
устанавливается animal как прототип для rabbit.

Затем, когда console.log пытается прочитать свойство rabbit.eats, его нет в rabbit, поэтому JavaScript следует по ссылке [[Prototype]] и находит его в animal (смотрите снизу вверх):

   animal__________ eats: true
   |         /|\ 
              |  [[Prototype]]
   rabbit_____|_____ jumps: true
...
rabbit2.walk();  // Animal walks // walk взят из прототипа animal2
...
longEar.walk();  // Animal walks // walk взят из цепочки прототипов - от animal2
console.log(longEar.jumps);  // true взят из цепочки прототипов - от rabbit2

/*
animal2 _____ eats: true, walk(){}
  /|\
   |  [[Prototype]]
rabbit2 _____ jumps: true
  /|\
   |  [[Prototype]]
longEar _____ earLength: 10

/*

Теперь, если мы прочтём что-нибудь из longEar, и оно будет отсутствовать, JavaScript будет искать его в rabbit2, а затем в animal2.
...
создадим объект существо - которые могут дышать
*/
let creature = {
    breathes: true
};

longEar.__proto__ = creature;

console.log(longEar.breathes);  // true , но он уже не наследует rabbit2, animal2

console.log(longEar.jumps); //  undefined
// longEar.walk();  // TypeError: longEar.walk is not a function
...
[[Prototype]] - есть у функции и у класса в JS
__proto__ - есть у всех объектов

у функции walk() - prototype есть, когда мы смотрим в консоли браузера.

Свойство __proto__ -  Это геттер/сеттер для [[Prototype]]. Позже мы увидим ситуации, когда это имеет значение, а пока давайте просто будем иметь это в виду, поскольку мы строим наше понимание языка JavaScript.

Свойство __proto__ немного устарело, оно существует по историческим причинам. Современный JavaScript предполагает, что мы должны использовать функции 
...
admin.fullName = "Kondrat Elkin";  // (**) срабатывает сеттер!
console.log(admin.name);  // Kondrat

console.log(admin.surname);  // Elkin
/*
...
rabbit6.sleep();  // // модифицирует rabbit.isSleeping

console.log(Object.keys(animal6));  // [ 'walk', 'sleep' ]

console.log(rabbit6.isSleeping);  // true
console.log(animal6.isSleeping);  // undefined  (нет такого свойства в прототипе)

animal6.sleep();

console.log(animal6.isSleeping);  // true

console.log(Object.keys(rabbit6))  // [ 'name', 'isSleeping' ]

/*
Картинка с результатом:

animal6  ___________ walk: function, sleep: function

rabbit6  ___________ name, isSleeping: true
...
Object.prototype (так как animal – литеральный объект {...}, то это по умолчанию), а затем null на самом верху:

 null 
  /|\
   |
Object.prototype  __________ toString: function, hasOwnProperty: function ...

  /|\
   |
animal ________________ eats: true
  /|\
   |
rabbit ________________ jumps: true



Заметим ещё одну деталь. Откуда взялся метод 
rabbit.hasOwnProperty? 
Мы его явно не определяли. Если посмотреть на цепочку прототипов, то видно, что он берётся из 
